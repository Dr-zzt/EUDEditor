...
- **Semantic DatEdit entries** — dat edits are stored as `{dat: units, field: Hit Points, id: 15, delta: 129}` instead of an uninterpretable number tuple, using the field names from the dat definitions.
...
- **의미 있는 DatEdit 저장** — dat 수정 내역이 해석 불가능한 숫자 튜플 대신 `{dat: units, field: Hit Points, id: 15, delta: 129}`처럼 dat 정의의 필드 이름으로 저장됩니다.
